...
class Analyzer(FileManager):
    ...
    def _count_non_alpha_chars(self, n=10):
        ...
        return {
            "10_most_common_with_freq": dict(non_alpha_chars.most_common(n)),
    ...
    def generate_distribution_plots(
        ...
        word_distribution = self._get_word_distribution(
            num_bins=num_bins, bins=bins_word
        )
        char_distribution = self._get_char_distribution(
            num_bins=num_bins, bins=bins_char
        )
    ...
    def _get_word_distribution(self, num_bins=10, bins=None):
        bins = (
            self._calculate_bins(df=self.data, num_bins=num_bins, column_name="n_word")
            if bins is None
            else bins
        )
    ...
    def _get_char_distribution(self, num_bins=10, bins=None):
        bins = (
            self._calculate_bins(df=self.data, num_bins=num_bins, column_name="n_char")
            if bins is None
            else bins
        )
