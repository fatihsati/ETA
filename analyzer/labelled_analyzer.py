...
from ETA.analyzer.file_manager import FileManager
from ETA.analyzer.plotter import Plotter
...
class LabelledAnalyzer(FileManager):
    ...
    def read_df(
        self, df: pd.DataFrame, text_column: str = "text", label_column="label"
    ):
        """
        Parameters
        ----------
    ...
    def generate_distribution_plots(
        ...
        series_list = [
            s
            for pair in zip(word_distribution_list, char_distribution_list)
            for s in pair
        ]
    ...
    def to_json(self, folder_name: str = "stats", filename_list=None):
        ...
        for class_, filename in zip(self.classes, filename_list):
            path = self._generate_path_from_filename(
                folder_name, filename, extension=".json"
            )
    ...
    def to_txt(self, folder_name: str = "stats", filename_list: list | None = None):
        ...
        for class_, filename in zip(self.classes, filename_list):
            path = self._generate_path_from_filename(
                folder_name, filename, extension=".txt"
            )
    ...
    def _get_filename_list(self, filename_list):
        ...
        if filename_list:
            if len(filename_list) != len(self.classes):
                raise ValueError(
                    f"Length of filename_list should be equal to the number of \
    ...
    def _generate_path_from_filename(self, folder_name, filename, extension=".json"):
        ...
        filename = (
            filename + extension if not filename.endswith(extension) else filename
        )
    ...
    def _calculate_bins_for_char(self):
        ...
        word_bins = self.analyze_objects[self.classes[0]]._calculate_bins(
            self.data, column_name="n_char"
        )
    ...
    def _check_given_classes(self, classes):
        ...
        for item in classes:  # check list elements are in classes
            if item not in self.classes:
                raise ValueError(
                    f"Class {item} not found in the data.\n\
